...
class controller_node(Node):
    ...
    def __init__(self, node_id, single_node, payload_params_path, uav_params_path, mechanism_params_path, payload_control_gain_path, uav_control_gain_path):
        ...
        if self.single_node:
            ...
            self.create_subscription(PositionCommand, '/payload/des_traj', self.desired_traj_callback)
            self.create_subscription(Odometry, '/payload/odom', self.pl_odom_callback)

            for uav_id in range(self.pl_params.nquad):
                mav_odom = mav_name + str(uav_id+1) + '/odom'
                self.qd[uav_id] = {}
                self.create_subscription(Odometry, mav_odom, lambda msg: self.qd_odom_callback(msg,uav_id))
    ...
    def assembly_FM_message(self, F_list, M_list, uav_id):
        ...
        if self.pl_params.mechanism_type == 'Rigid Link':
            ...
            if self.pl_params.payload_type == 'Rigid Body':
                ...
                FM_message.moments.x = M_list[0,0][0].item()
                FM_message.moments.y = M_list[1,0][0].item()
                FM_message.moments.z = M_list[2,0][0].item()
                return FM_message
